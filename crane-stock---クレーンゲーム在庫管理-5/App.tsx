...
const App: React.FC = () => {
  ...
  const finalizeImport = useCallback(async (mode: 'append' | 'replace') => {
    ...
    try {
      ...
      try {
        ...
        await Promise.all([
          StorageService.savePrizes(nextPrizes),
          StorageService.saveSpendingRecords(nextSpending)
        ]);
        setIsDirty(false);
        setSaveStatus('saved');
        setTimeout(() => setSaveStatus('idle'), 2000);
        alert(mode === 'replace' ? 'データを完全に置き換えました。' : '既存データに統合しました。');
      } catch (error) {
  ...
  const handleSaveSpending = useCallback(async (record: SpendingRecord) => {
    setSpendingRecords(prev => {
      ...
      StorageService.saveSpendingRecord(record).catch(err => console.error("Spending save failed", err));
      return next;
    });
  }, []);
  ...
  const handleDeleteSpending = useCallback(async (id: string) => {
    if (confirm('この記録を削除しますか？')) {
      setSpendingRecords(prev => {
        ...
        StorageService.deleteSpendingRecord(id).catch(err => console.error("Spending delete failed", err));
        return next;
      });
      setIsDirty(true);
    }
  ...
  const handleSavePrize = useCallback(async (prize: Prize) => {
    ...
    setPrizes(prevPrizes => {
      ...
      // 非同期で保存を実行
      StorageService.savePrize(updatedPrize).catch(err => console.error("Auto-save failed", err));
  ...
  const handleDeletePrize = useCallback((prizeId: string) => {
    if (confirm('この景品を削除してもよろしいですか？')) {
      setPrizes(prevPrizes => {
        ...
        StorageService.deletePrize(prizeId).catch(err => console.error("Auto-delete failed", err));
        return next;
      });
    }
  }, []);
    ...
    setPrizes(prevPrizes => {
      const next = prevPrizes.map(p => {
        if (p.id === prizeId) {
          ...
          const updated = { ...p, quantity: newQuantity, history: [...(p.history || []), historyEntry].slice(-100) };
          StorageService.savePrize(updated).catch(err => console.error("Auto-save failed", err));
          return updated;
        }
        return p;
      });
    ...
    setPrizes(prevPrizes => {
      const next = prevPrizes.map(p => {
        if (p.id === prizeId && p.quantity > 0) {
          ...
          const updated = { ...p, quantity: p.quantity - 1, history: [...(p.history || []), historyEntry].slice(-100) };
          StorageService.savePrize(updated).catch(err => console.error("Auto-save failed", err));
          return updated;
        }
        return p;
                ...
                <button 
                  onClick={() => {
                    if (confirm('すべてのデータを削除しますか？この操作は取り消せません。')) {
                      ...
                      setSpendingRecords([]);
                      setIsDirty(true);
                      StorageService.clearAll().then(() => alert('全データを削除しました。'));
                    }
                  }}
